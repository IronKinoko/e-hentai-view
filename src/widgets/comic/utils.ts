...
export const pageDataCache = new Map<string, ComicListDataSourceProps>()

export async function getPageWithRetry(
  url: string
): Promise<ComicListDataSourceProps> {
  try {
    if (pageDataCache.has(url)) return pageDataCache.get(url)!

    let res = await axios.get<
      ComicListDataSourceProps & { error: boolean; message: string }
    >(url)
